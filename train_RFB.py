...
from data import ANPRDetection, ANPRAnnotationTransform, COCO_300, COCO_512
...
def train():
    ...
    if args.dataset == 'VOC':
        ...
        dataset = ANPRDetection('../data',preproc(
            img_dim, rgb_means, p), ANPRAnnotationTransform())
    else:
    ...
    stepvalues_VOC = (200 * epoch_size, 300 * epoch_size, 400 * epoch_size, 500 * epoch_size)
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration >= stepvalues[step_index]:
            step_index += 1
...
def adjust_learning_rate(optimizer, gamma, epoch, step_index, iteration, epoch_size):
    ...
    """
    if epoch < 10:
        lr = 1e-6 + (args.lr-1e-6) * iteration / (epoch_size * 9) 
    else:
